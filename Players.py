import random
from Functions import doTheyDeduce, doTheyDefend, freeRest, whoHere, workload
...
class Player:
    ...
    def KILL(self, target, report, time, locations, players, weapons):
        ...
        if defend is "pass":
            if target.weapon is not weapons[17]:
        ...
        if self.weapon is weapons[0]:               #LIFTING WEIGHT
        ...
        if self.currentWeapon.type is "blunt":      #Blunt weapon fight
            if self.weapon is not weapons[9]:
                self.marks.append("bruises")
            if target.weapon is not weapons[9]:
            ...
            if self.strength > target.strength:
                ...
                if target.weapon is weapons[13]:
                    self.alive = False
                    ...
                    if target.honor is 0 and self.honor < 0:
                        target.honor = 1
                    if target.honor is 0 and self.honor > 0:
                        target.honor = -1
